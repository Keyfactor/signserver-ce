...
public class CertificateDetailsBean {
    ...
    private List<String> detailedListSelection;
    ...
    public List<String> getDetailedListSelection() {
        if (detailedListSelection == null) {
            detailedListSelection = Arrays.asList(getDetailed().getFriendlyName());
        }
        return detailedListSelection;
    }

    public void setDetailedListSelection(List<String> detailedListSelection) {
        this.detailedListSelection = detailedListSelection;
    }

    public void viewSelectedAction() {

        if (!detailedListSelection.isEmpty()) {
            // Use the first selection
            String selectedName = detailedListSelection.get(0);

            // Find selected friendly name
            for (DetailedCertificate dc : getDetailedList()) {
                if (dc.getFriendlyName().equals(selectedName)) {
                    detailed = dc;
                    break;
                }
            }
        ...
        detailedListSelection = Arrays.asList(detailed.getFriendlyName());
    }

    public void exportSelectedAction() throws IOException, AdminNotAuthorizedException, NoSuchWorkerException {
        if (detailedList.size() == 1 || !detailedListSelection.isEmpty()) {
            // Get selection and in right order
            List<Certificate> certs = new ArrayList<>();
            if (detailedList.size() == 1) {
                certs.add(detailedList.get(0).getCertificate());
            } else {
                for (DetailedCertificate dc : getDetailedList()) {
                    if (detailedListSelection.contains(dc.getFriendlyName())) {
            ...
            externalContext.setResponseHeader("Content-Disposition", "attachment; filename=\"" + getWorker().getName() + "-certificate" + (withChain ? "-chain" : "") + FILE_SUFFIX);
