...
class SignApk {
    ...
    static Manifest addDigestsToManifest(JarFile jar, List<String> hashes, String createdBy)
        ...
        for (String hash : hashes) {
            mds.add(MessageDigest.getInstance(hash));
        }
    ...
    private static void writeSignatureFile(Manifest manifest, OutputStream out,
                                           X509Certificate[] certificateChain, String digestAlgorithm, String createdBy)
        throws IOException, GeneralSecurityException {
        ...
        if (digestAlgorithm == null) {
            digestAlgorithm = (getDigestAlgorithm(certificateChain[0]) == USE_SHA256) ? "SHA-256" : "SHA1";
        }
     ...
     * more efficient.
     */
    static void copyFiles(Manifest manifest, JarFile in, JarOutputStream out,
                                  long timestamp, int alignment, boolean keepSignatures, boolean replaceSignature, String signatureName) throws IOException {
        byte[] buffer = new byte[4096];
        int num;
        Map<String, Attributes> entries = manifest.getEntries();
        ArrayList<String> names = new ArrayList<>(entries.keySet());
        
        if (keepSignatures || replaceSignature) {
            Enumeration<JarEntry> jarEntries = in.entries();
            while (jarEntries.hasMoreElements()) {  // XXX: Performance: iterates over all entries again
                JarEntry entry = jarEntries.nextElement();
                String name = entry.getName().toUpperCase(Locale.ENGLISH);
                // If signature should be replaced and this is the .SF file
                // OR if the name starts with ie "META-INF/CERT." AND ends with RSA|DSA|EC
                // ELSE if signatures should be keept and this is one such file
                if (replaceSignature && (
                        createSFName(1, 0, signatureName).equalsIgnoreCase(name)
        ...
        public void write(OutputStream out) throws IOException {
            try {
                ...
                try (JarOutputStream outputJar = new JarOutputStream(signer)) {
                    ...
                    copyFiles(manifest, inputJar, outputJar, timestamp, 0, keepSignatures, replaceSignature, signatureName);
    ...
    static void signFile(Manifest manifest, JarFile inputJar,
                                 ...
                                 Provider provider,
                                 JarOutputStream outputJar, String signatureAlgorithm, String digestAlgorithm, TimeStampingProvider timeStamping, boolean keepSignatures, String signatureName, String createdBy) throws IOException, GeneralSecurityException, CertificateEncodingException, OperatorCreationException, CMSException
         {
        // Assume the certificate is valid for at least an hour.
        ...
        outputJar.putNextEntry(mfEntry);
        ...
        for (int k = 0; k < numKeys; ++k) {
            ...
            writeSignatureFile(manifest, baos, certificateChains[k], digestAlgorithm, createdBy);
