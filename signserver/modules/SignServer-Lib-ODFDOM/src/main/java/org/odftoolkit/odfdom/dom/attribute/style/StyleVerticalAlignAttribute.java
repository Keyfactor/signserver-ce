...
public class StyleVerticalAlignAttribute extends OdfAttribute {
		...
		if( parentElement != null ){
			try {
				if( parentElement instanceof StyleColumnSepElement ){
					super.setValue(StyleColumnSepElement.StyleVerticalAlignAttributeValue.enumValueOf(attrValue).toString());
				}else if( parentElement instanceof StyleParagraphPropertiesElement ){
					super.setValue(StyleParagraphPropertiesElement.StyleVerticalAlignAttributeValue.enumValueOf(attrValue).toString());
		...
		if( parentElement != null ){
			try {
				if( parentElement instanceof StyleColumnSepElement ){
					...
					return StyleTableCellPropertiesElement.StyleVerticalAlignAttributeValue.enumValueOf(super.getValue()).toString();
				}			
			} catch (IllegalArgumentException e) {
				// TODO: validation handling/logging
				throw new NumberFormatException("the value of style:vertical-align is not valid");
			}
		}else{
			//2DO:what should do when the owner element is null?
	...
	/**
	 * Returns the default value of {@odf.attribute style:vertical-align} dependent of its element name
	 * 
