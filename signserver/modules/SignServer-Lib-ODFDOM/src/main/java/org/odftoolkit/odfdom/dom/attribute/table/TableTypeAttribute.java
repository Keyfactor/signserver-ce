		...
		if( parentElement != null ){
			try {
				if( parentElement instanceof TableDataPilotFieldReferenceElement ){
					super.setValue(TableDataPilotFieldReferenceElement.TableTypeAttributeValue.enumValueOf(attrValue).toString());
				}else if( parentElement instanceof TableDeletionElement ){
					super.setValue(TableDeletionElement.TableTypeAttributeValue.enumValueOf(attrValue).toString());
				}else if( parentElement instanceof TableInsertionElement ){
					super.setValue(TableInsertionElement.TableTypeAttributeValue.enumValueOf(attrValue).toString());
				}			
		...
		if( parentElement != null ){
			try {
				if( parentElement instanceof TableDataPilotFieldReferenceElement ){
					...
					return TableInsertionElement.TableTypeAttributeValue.enumValueOf(super.getValue()).toString();
				}			
			} catch (IllegalArgumentException e) {
				// TODO: validation handling/logging
				throw new NumberFormatException("the value of table:type is not valid");
			}
		}else{
			//2DO:what should do when the owner element is null?
