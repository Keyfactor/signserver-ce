...
package org.signserver.lib.itext.text.html.simpleparser;
...
import org.signserver.lib.itext.text.html.HtmlTags;
import org.signserver.lib.itext.text.html.Markup;
...
import org.signserver.lib.itext.text.ElementTags;
import org.signserver.lib.itext.text.ExceptionConverter;
import org.signserver.lib.itext.text.FontFactoryImp;
...
import org.signserver.lib.itext.text.TextElementArray;
import org.signserver.lib.itext.text.pdf.PdfPTable;
import org.signserver.lib.itext.text.pdf.draw.LineSeparator;
import org.signserver.lib.itext.text.xml.simpleparser.SimpleXMLDocHandler;
import org.signserver.lib.itext.text.xml.simpleparser.SimpleXMLParser;
	...
	public void startElement(String tag, HashMap h) {
		...
		try {
			...
			if (tag.equals(HtmlTags.UNORDEREDLIST)) {
				...
				skipText = true;
				cprops.addToChain(tag, h);
				org.signserver.lib.itext.text.List list = new org.signserver.lib.itext.text.List(false);
				try{
			...
			if (tag.equals(HtmlTags.ORDEREDLIST)) {
				...
				skipText = true;
				cprops.addToChain(tag, h);
				org.signserver.lib.itext.text.List list = new org.signserver.lib.itext.text.List(true);
				try{
					list.setIndentationLeft(new Float(cprops.getProperty("indent")).floatValue());
				}catch (Exception e) {
					list.setAutoindent(true);
		...
		try {
			...
			if (tag.equals(HtmlTags.UNORDEREDLIST)
				...
				if (stack.empty())
					return;
				Object obj = stack.pop();
				if (!(obj instanceof org.signserver.lib.itext.text.List)) {
					stack.push(obj);
					return;
				}
				if (stack.empty())
			...
			if (tag.equals(HtmlTags.LISTITEM)) {
				...
				if (!(list instanceof org.signserver.lib.itext.text.List)) {
					stack.push(list);
					return;
				}
				ListItem item = (ListItem) obj;
				((org.signserver.lib.itext.text.List) list).add(item);
