...
public class ValidationUtils {
    ...
    public static int getReasonCodeFromCRLEntry(X509CRLEntry crlEntry)
            throws IOException {
        // retrieve reason
        byte[] reasonBytes = crlEntry.getExtensionValue(Extension.reasonCode.getId());
        if (reasonBytes == null) {
            // if null then unspecified (RFC 3280)
            return CRLReason.unspecified;
        }

        ASN1Enumerated reasonCode = (ASN1Enumerated) X509ExtensionUtil.fromExtensionValue(reasonBytes);
