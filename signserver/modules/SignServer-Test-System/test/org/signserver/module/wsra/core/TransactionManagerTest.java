...
public class TransactionManagerTest extends CommonManagerT {
    ...
    public void test01BasicUserManager() throws Exception {
        ...
        tb();
        int id = tm1.addTransaction(t1);
        tc();
        assertTrue(id != 0);
        tb();
        int id2 = tm1.addTransaction(t2);
        tc();
        assertTrue(id2 != 0 && id != id2);
        tb();
        tm1.addTransaction(t3);
        tc();
        tb();
        tm1.addTransaction(t4);
        tc();
        ...
        tb();
        result = tm1.getUnprocessed(startDate, endDate);
        tc();
        assertTrue(result.size() == 4);
        ...
        tb();
        tm1.markAsProcessed(startDate, endDate);
        tc();
        result = tm1.listTransactions(startDate, endDate, WSRAConstants.TRANSACTIONSTATUS_PROCESSED);
        assertTrue(result.size() == 4);
        for (TransactionDataBean r : result) {
        ...
        tb();
        tm1.addTransaction(t5);
        tc();
        tb();
        tm1.addTransaction(t6);
        tc();
        tb();
        result = tm1.getUnprocessed(startDate, endDate);
        tc();
        ...
        tb();
        tm1.addTransaction(t7);
        tc();
        tb();
        tm1.addTransaction(t8);
        tc();
        tb();
        result = tm2.getUnprocessed(startDate, endDate);
        tc();
        assertTrue(result.size() == 2);
        assertTrue(result.get(0).getNodeId() == "node2".hashCode());

        tb();
        tm2.markAsProcessed(startDate, endDate);
        tc();
        ...
        tb();
        tm1.removeTransactions(startDate, endDate);
        tc();
