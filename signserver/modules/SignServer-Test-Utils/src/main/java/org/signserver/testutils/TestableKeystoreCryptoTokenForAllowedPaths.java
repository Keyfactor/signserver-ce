/*************************************************************************
 *                                                                       *
 *  SignServer: The OpenSource Automated Signing Server                  *
 *                                                                       *
 *  This software is free software; you can redistribute it and/or       *
 *  modify it under the terms of the GNU Lesser General Public           *
 *  License as published by the Free Software Foundation; either         *
 *  version 2.1 of the License, or any later version.                    *
 *                                                                       *
 *  See terms of license at gnu.org.                                     *
 *                                                                       *
 *************************************************************************/

package org.signserver.testutils;

import org.signserver.server.cryptotokens.KeystoreCryptoToken;

import java.io.FileNotFoundException;
import java.nio.file.Path;
import java.util.Set;


/**
 * Test-only subclass of KeystoreCryptoToken that provides mock behavior
 * for allowed keystore path validation during unit testing.
 */
public class TestableKeystoreCryptoTokenForAllowedPaths extends KeystoreCryptoToken {

    @Override
    protected Set<Path> getAllowedKeystorePaths() {
        try {
            return Set.of(Path.of(ModulesTestCase.getSignServerHome().getAbsolutePath()).normalize());
        } catch (FileNotFoundException ignored) {}
        return Set.of();
    }
}
