...
public class GroupKeyDataService {
	...
	public GroupKeyDataBean fetchKey(String documentId, boolean genKeyIfNotExist) throws CryptoTokenOfflineException, IllegalRequestException, SignServerException{
		...
		// Find Key
	    Collection<?> groupKeyResult = em.createNamedQuery("GroupKeyDataBean.findByDocumentId")
        .setParameter(1, this.workerId)
	    ...
	    if(groupKeyResult.size() > 0){
	    	...
	    	if(genKeyIfNotExist){
	    	    ...
	    	    List<?> result = em.createNamedQuery("GroupKeyDataBean.findUnassignedKey")
	            .setParameter(1, this.workerId)
	            .setMaxResults(1)
	            .getResultList();
	    		
	    		if(!usePregeneration() || result.size() == 0){
	    			// Generate new Key
	...
	private GroupKeyDataBean genNewGroupKeyData() throws CryptoTokenOfflineException, SignServerException {
		...
		Collection<?> encKeyResult = em.createNamedQuery("EncKeyDataBean.findByUseFlag")
		.setParameter(1, this.workerId)
		.getResultList();
...
//			Encryption key doesn't exist, create one.
	...
	public long getNumOfUnassignedKeys(Date startDate, Date endDate){
		Number result =  (Number) em.createNamedQuery("GroupKeyDataBean.numberOfUnassignedKeys")
        .setParameter(1, this.workerId)
	...
	public long getNumOfAssignedKeys(Date startDate, Date endDate){
		 Number result =  (Number) em.createNamedQuery("GroupKeyDataBean.numberOfAssignedKeys")
         .setParameter(1, this.workerId)
         .setParameter(2, startDate)
         .setParameter(3, endDate)
         .getSingleResult();
	...
	public long getNumOfKeys(Date startDate, Date endDate){
		 Number result =  (Number) em.createNamedQuery("GroupKeyDataBean.totalNumberOfKeys")
         .setParameter(1, this.workerId)
         .setParameter(2, startDate)
         .setParameter(3, endDate)
         .getSingleResult();
	 ...
	 */
	private long keySwitchThreshold = 0;
	...
	private String groupKeyAlg = null;
	...
	private String groupKeySpec = null;
	...
	private String encKeySpec = null;
	...
	private Boolean usePregeneration = null;
