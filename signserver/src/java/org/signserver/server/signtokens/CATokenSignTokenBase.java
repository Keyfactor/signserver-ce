 ...
 * as sign tokens in Signserver.
 * 
 * @see org.signserver.server.signtokens.ISignToken
 * @author Philip Vendil, Tomas Gustavsson
 * @version $Id: CATokenSignTokenBase.java,v 1.1 2007-09-27 10:02:27 anatom Exp $
 */
public abstract class CATokenSignTokenBase implements ISignToken{

	private static final Logger log = Logger.getLogger(CATokenSignTokenBase.class);
	...
	/**
	 * Method activating the sign token using the given key
	 * 
	 * @throws SignTokenAuthenticationFailureException if activation failed, message gives more info
	 * @throws SignTokenOfflineException if connection to token could not be created.
	 * 
	 */
	 ...
	 * @see ISignToken.PROVIDERUSAGE_SIGN
	 */
	...
	public ISignerCertReqData genCertificateRequest(ISignerCertReqInfo info) throws SignTokenOfflineException {
		...
		if(info instanceof PKCS10CertReqInfo){
			...
			try {
				pkcs10 = new PKCS10CertificationRequest(reqInfo.getSignatureAlgorithm(),CertTools.stringToBcX509Name(reqInfo.getSubjectDN()),getPublicKey(PURPOSE_SIGN),reqInfo.getAttributes(),getPrivateKey(PURPOSE_SIGN),getProvider(ISignToken.PROVIDERUSAGE_SIGN));
