...
public class OCSPCRLPathChecker extends OCSPPathChecker {
	...
	public void check(Certificate cert, Collection<String> unresolvedCritExts)
		...
		try {
			oCSPURLString = CertTools
					.getAuthorityInformationAccessOcspUrl(x509Cert);
		} catch (CertificateParsingException e1) {
			// eat exception since we are going to fail over to crl
			log
					.debug("exception on retrieving OCSP url from certificate, will fail over to CRL");
			failOverToCRL = true;
		}
		...
		if (oCSPURLString == null || oCSPURLString.length() == 0) {
			...
			log
					.debug("OCSP url is not present in certificate, will fail over to CRL");
			failOverToCRL = true;
		}
		...
		if (failOverToCRL) {
			...
			try {
				...
				log
						.debug("exception on retrieving CDP url from certificate, will use preconfigured CRLPATHS");
			...
			if (crlURL == null) {
				// OCSP url does not exist
				log
						.debug("CDP url is not present in certificate, will use preconfigured CRLPATHS");
				useConfiguredCRLPaths = true;
			}
	...
	private void parseAndVerifyCRL(X509Certificate x509Cert, URL crlURL)
		...
		if (crlEntry != null) {
			...
			if (crlEntry.hasExtensions()) {
				...
				try {
					reasonCode = ValidationUtils
							.getReasonCodeFromCRLEntry(crlEntry);
				} catch (IOException e) {
		...
		try {
			...
			if (cACert != null) {
				CertificateFactory certFact = CertificateFactory.getInstance(
						"X509", "BC");
				ByteArrayInputStream bis = new ByteArrayInputStream(cACert
						.getEncoded());
				clonedPrevCert = (X509Certificate) certFact
