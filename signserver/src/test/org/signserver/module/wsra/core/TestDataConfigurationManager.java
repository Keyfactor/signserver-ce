...
public class TestDataConfigurationManager extends CommonManagerTest {
	...
	private void setDB() throws Exception{
		...
		tb();pm.editProduct(pdb1);tc();
		tb();pm.editProduct(pdb2);tc();
		...
		tb();pm.editPrice(prdb1);tc();
		tb();pm.editPrice(prdb2);tc();
		...
		tb();om.editProductInOrganization(piod1);tc();
		ProductsInOrganizationDataBean piod2= new ProductsInOrganizationDataBean("2345","budget","SEK");
		piod2.setOrganizationId(orgId1);
		tb();om.editProductInOrganization(piod2);tc();
		
		tb();dbm.setRelatedProperty(WSRAConstants.DATABANKTYPE_ORGANIZATION, orgId1, "testkey1", "testvalue1");tc();
		tb();dbm.setRelatedProperty(WSRAConstants.DATABANKTYPE_ORGANIZATION, orgId1, "testkey2", "testvalue2");tc();
		...
		tb();tm.editToken(t1);tc();
		tb();tm.editToken(t2);tc();		
