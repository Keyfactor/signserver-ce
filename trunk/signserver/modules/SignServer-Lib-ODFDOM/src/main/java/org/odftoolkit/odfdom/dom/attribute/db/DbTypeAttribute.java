		...
		if( parentElement != null ){
			try {
				if( parentElement instanceof DbKeyElement ){
					super.setValue(DbKeyElement.DbTypeAttributeValue.enumValueOf(attrValue).toString());
				}else if( parentElement instanceof DbServerDatabaseElement ){
					//2DO: need validate value against NamespacedToken
					super.setValue(attrValue);
				}else if( parentElement instanceof DbTableDefinitionElement ){
					super.setValue(attrValue);
				}			
	...
	public String getValue(){
		...
		if( parentElement != null ){
			try {
				...
				// TODO: validation handling/logging
				throw new NumberFormatException("the value of db:type is not valid");
			}
		}else{
			//2DO:what should do when the owner element is null?
			return super.getValue();
