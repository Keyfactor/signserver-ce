...
/**
 * Authorizer checking JSON Web Tokens (JWT) provided by the clients.
 * 
 * @see RequestContext#CLIENT_CREDENTIAL_BEARER
 *
...
public class JwtAuthorizer implements IAuthorizer {
    ...
    // Worker properties
    private final String AUTH_SERVER_PREFIX = "AUTHSERVER";
    ...
    private final String ACCEPTED_AUDIENCES = "ACCEPTED_AUDIENCES";
    ...
    private final Set<String> acceptedAudiences = new HashSet<>();
    ...
    public void init(int workerId, WorkerConfig config, EntityManager em)
        ...
        final String acceptedAudiencesString =
                config.getProperties().getProperty(ACCEPTED_AUDIENCES);
        if (StringUtils.isNotBlank(acceptedAudiencesString)) {
            for (final String part : acceptedAudiencesString.split(",")) {
                final String audience = part.trim();
    ...
    protected final Jws<Claims> validateToken(String token) throws JwtException {
        ...
        final Object audience = jws.getBody().get(Claims.AUDIENCE);
        if (audience != null) {
            if (audience instanceof List) {
                final List<String> audiences = (List<String>) audience;
                for (final String aud : audiences) {
                    if (acceptedAudiences.contains(aud)) {
                        return jws;
                    }
                }
            } else {
                if (acceptedAudiences.contains(audience.toString())) {
                    return jws;
                }
            }

            throw new JwtException("Specific audience specified not among accepted: " + audience.toString());
        }
