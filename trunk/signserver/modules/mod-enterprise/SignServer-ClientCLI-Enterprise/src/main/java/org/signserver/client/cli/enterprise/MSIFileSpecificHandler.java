...
public class MSIFileSpecificHandler extends AbstractFileSpecificHandler {
    ...
    @Override
    public InputSource produceSignatureInput(String algorithm) throws NoSuchAlgorithmException, IOException, IllegalRequestException, NoSuchProviderException {
        FileUtils.copyFile(getInFile(), getOutFile(), false);
        msiFile = closeLater(new MSIFile(getOutFile()));
    ...
    public void assemble(final OutputCollector oc) throws IOException {
        try (final POIFSFileSystem fsOut = new POIFSFileSystem(getOutFile(), false)) { // TODO: move to field and handle in close()
            ...
            fsOut.createOrUpdateDocument(new ByteArrayInputStream(signature.toASN1Structure().getEncoded("DER")),"\05DigitalSignature");
