...
public class ApkLineageSigner extends BaseSigner {
    ...
    public Response processData(Request signRequest,
        ...
        try {
            ...
            } catch (NoSuchAlgorithmException | NoSuchProviderException | CertificateEncodingException ex) {
    ...
    private static void printCertificate(final X509Certificate cert, final String name,
        ...
        try (final PrintWriter writer = new PrintWriter(out)) {
            if (sha256 == null || sha1 == null || md5 == null) {
                sha256 = MessageDigest.getInstance("SHA-256", BouncyCastleProvider.PROVIDER_NAME);
                sha1 = MessageDigest.getInstance("SHA-1", BouncyCastleProvider.PROVIDER_NAME);
                md5 = MessageDigest.getInstance("MD5", BouncyCastleProvider.PROVIDER_NAME);
            }
