...
public class AppxCMSSignerUnitTest {
    ...
    private void signAndVerify(final File file,
        ...
        try (
            ...
            assertTrue("Verification using signer certificate",
                    si.verify(new JcaSignerInfoVerifierBuilder(new AuthenticodeDigestCalculatorProvider()).build(configuredSignerCert)));
    ...
    private byte[] createSignedContent(final DigestInfo digestInfo)
            throws IOException {
        final SpcSipInfo sipInfo = AppxCMSSigner.createAppxSpcSipInfo();
        ...
        System.out.println("signed content: " + Hex.toHexString(signedContent));
