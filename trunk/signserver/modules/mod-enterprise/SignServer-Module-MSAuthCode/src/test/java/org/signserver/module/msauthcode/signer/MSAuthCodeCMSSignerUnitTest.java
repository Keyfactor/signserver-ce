...
public class MSAuthCodeCMSSignerUnitTest {
    ...
    public void testInit_contentOIDNotAllowed() throws Exception {
        ...
        final WorkerConfig config = new ConfigBuilder().create();
    ...
    /**
     * Test signing with an incorrect TSA_URL to see that the failure is correct.
     *
     * @throws Exception 
     */
    @Test
    public void testSignPEWithIncorrectTSAURL() throws Exception {
        final WorkerConfig config = new WorkerConfig();
        config.setProperty("TSA_URL", "http://localhost:8080/nonexisting");
        ...
        try {
            signAndVerify(peFile, false, "SHA256", config, requestContext, tokenRSA);
            fail("Should have failed with SignServer Exception");
        } catch (SignServerException ex) {
            assertEquals("Error constructing CMS", ex.getMessage()); // Note: Might need change in future when we use a more proper error message
        }
    }
     ...
     */
    @Test
    public void testSignMSIWithIncorrectTSAURL() throws Exception {
        final WorkerConfig config = new WorkerConfig();
        config.setProperty("TSA_URL", "http://localhost:8080/nonexisting");
    ...
    private byte[] createSignedContent(final DigestInfo digestInfo,
        ...
        try {
            ASN1InputStream in = new ASN1InputStream(idcBytes);
            ASN1Sequence sequence = (ASN1Sequence) in.readObject();
            for (ASN1Encodable element : sequence) {
                bout.write(element.toASN1Primitive().getEncoded());
            }
        } catch (IOException e) {
