...
public class MasterListSignerUnitTest {
    ...
    public void testSigningErrorNoSKID() throws Exception {
     ...
     /**
     * Test that trying to sign with signature algorithm parameter specified as empty value works.
     *
     * @throws Exception
     */
    @Test
    public void testSigningEmptySigAlgo() throws Exception {
        LOG.info("testSigningEmptySigAlgo");

        WorkerConfig config = new WorkerConfig();
        config.setProperty(MasterListSigner.SIGNATUREALGORITHM_PROPERTY, "   ");
        MasterListSigner instance = new MockedMasterListSigner(tokenRSA);
        instance.init(1, config, new SignServerContext(), null);
