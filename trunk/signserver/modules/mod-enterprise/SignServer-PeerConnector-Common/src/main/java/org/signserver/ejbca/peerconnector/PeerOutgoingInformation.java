...
public class PeerOutgoingInformation implements Serializable, Comparable<PeerOutgoingInformation> {
    ...
    public PeerOutgoingInformation(final int id, final String name, final PeerState state, final String url, final String persistenceData) {
        this.id = id;     
        this.name = name.trim();
        this.state = state;
        this.url = url.trim();
    ...
    private void parsePersistenceData(final String persistenceData) {
        XMLDecoder decoder;
        try {
            decoder = new XMLDecoder(new ByteArrayInputStream(persistenceData.getBytes("UTF8")));
