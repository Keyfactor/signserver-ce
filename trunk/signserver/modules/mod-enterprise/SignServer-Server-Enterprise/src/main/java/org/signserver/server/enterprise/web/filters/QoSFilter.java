...
import static org.signserver.server.enterprise.web.filters.QoSFilterProperties.QOS_FILTER_ENABLED;
import static org.signserver.server.enterprise.web.filters.QoSFilterProperties.QOS_PRIORITIES;
import static org.signserver.server.enterprise.web.filters.QoSFilterProperties.QOS_MAX_REQUESTS;
import static org.signserver.server.enterprise.web.filters.QoSFilterProperties.QOS_MAX_PRIORITY;
import static org.signserver.server.enterprise.web.filters.QoSFilterProperties.QOS_CACHE_TTL_S;
import static org.signserver.server.enterprise.web.filters.QoSFilterProperties.QOS_PRIORITY;
import org.signserver.web.common.filters.ExtensionFilterImpl;
 ...
 * @version $Id$
 */
public class QoSFilter implements ExtensionFilterImpl {
    ...
    private static class State {
        ...
        private Set<Integer> highestPriorityWorkerIds; // All workers at the highest priority level
    }
    ...
    @Override
    public void init(final FilterConfig filterConfig, IServices services) throws ServletException {
        LOG.info("Loading QoS support");
        workerSession = services.get(WorkerSessionLocal.class);
        globalSession = services.get(GlobalConfigurationSessionLocal.class);
        if (workerSession == null || globalSession == null) {
            throw new ServletException("Required session beans not injected into Filter");
        }
    ...
    protected void recreateGlobalPropertyCache() {
        ...
        state.highestPriorityWorkerIds = createSetOfHighestPriorityWorkers(state.workerPriorities, state.maxPriorityLevel);
        if (LOG.isDebugEnabled()) {
            LOG.debug("Workers at highest priority level are: " + state.highestPriorityWorkerIds);
        }
    }
    
    private static Set<Integer> createSetOfHighestPriorityWorkers(Map<Integer, Integer> workerToPriorityMap, int maxPriorityLevel) {
        HashSet<Integer> results = new HashSet<>();
        for (Integer worker : workerToPriorityMap.keySet()) {
            if (workerToPriorityMap.get(worker) == maxPriorityLevel) {
                results.add(worker);
            }
        }
        return results;
    ...
    protected void doFilterWithPriorities(
        ...
        synchronized (state.globalPropertyCache) {
            ...
            if("/PriorityClientWS".equals(((HttpServletRequest) request).getServletPath())) {
                // Put in list of all workers at highest priority level so that
                // the WS implementation can later check that this request should
                // have been at higest priority level.
                request.setAttribute(RequestContext.QOS_PRIORITY_WORKER_ID_LIST, new HashSet<>(state.highestPriorityWorkerIds));
            }
        }
