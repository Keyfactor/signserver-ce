...
public class VerifyLogCommandTest extends ModulesTestCase {
    ...
    public void setUp() throws Exception {
        final File conf = new File(getSignServerHome(), "conf/databaseprotection.properties");
        if (!conf.exists()) {
            throw new IOException("No databaseprotection config file available: " + conf.getAbsolutePath());
        }
        Security.addProvider(new BouncyCastleProvider());
        ConfigurationHolder.addConfigurationFile(conf.getAbsolutePath());
    }
    ...
    private void fillLogServer1MissingSequence() {
    ...
    private void clearLog() throws Exception {
        ...
        final int deletedCount = em.createQuery("DELETE FROM AuditRecordData a where a.nodeId=:node1 or a.nodeId=:node2").setParameter("node1", SERVER1).setParameter("node2", SERVER2).executeUpdate();
    ...
    private void log(final String nodeId, final Long sequenceNumber, final String searchDetail1) {
        log(new Date(), searchDetail1, nodeId, sequenceNumber);
    }

    private void log(final Date time,
                     final String searchDetail1,
                     final String nodeId, final Long sequenceNumber) throws AuditRecordStorageException {
        if (LOG.isTraceEnabled()) {
            LOG.trace(String.format(">log:%s:%s:%s:%s:%s:%s", LogEventType.RELOAD_WORKER_CONFIG, EventStatus.SUCCESS, LogModuleType.SERVICE, LogServiceType.SIGNSERVER, "CLI User", null));
        }
        try {
            final EntityManager em = getEntityManager();
            em.getTransaction().begin();
            final Long timeStamp = time.getTime();
            final AuditRecordData auditRecordData = new AuditRecordData(
                    nodeId, sequenceNumber, timeStamp, LogEventType.RELOAD_WORKER_CONFIG, EventStatus.SUCCESS,
                    "CLI User", LogServiceType.SIGNSERVER, LogModuleType.SERVICE, null,
                    searchDetail1, null, null
            );
    ...
    public void testProtectionConfigured() throws Exception {
        ...
        if (!CesecoreConfiguration.useDatabaseIntegrityProtection("AuditRecordData")) {
