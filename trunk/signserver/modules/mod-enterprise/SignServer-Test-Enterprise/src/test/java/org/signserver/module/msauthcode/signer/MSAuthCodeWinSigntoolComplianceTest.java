...
public class MSAuthCodeWinSigntoolComplianceTest {
        ...
        try {
            ...
            if (inputFile.getName().endsWith(".msi")) {
                ...
                signedBinary = MSAuthCodeSignerTest.signAndAssertOkPs1(inputFile,
                        ...
                        useRFCTimestamper,
                        false, 1);
            } else if (inputFile.getName().endsWith(".cat")) {
                signedBinary = MSAuthCodeSignerTest.signAndAssertOkCat(inputFile,
                        WORKER_ID, TS_ID, time, useRFCTimestamper, 1);
            } else {
                if (doResign) {
                    signedBinary =
    ...
    private void signThenVerifyInternalClientHashing(final boolean doResign)
        ...
        // we don't support client-side hashing for CAT
        Assume.assumeFalse("Client-side hashing not supported for CAT",
                           inputFile.getName().endsWith(".cat"));
