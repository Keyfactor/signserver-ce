...
public class QoSFilterTest extends ModulesTestCase {
    ...
    public static void tearDownClass() throws Exception {
        ...
        GLOBAL_SESSION.setProperty(GlobalConfiguration.SCOPE_GLOBAL, QoSFilterProperties.QOS_FILTER_ENABLED, "true");
        
        // wait until old cached filter config has expired (with some margin)
        Thread.sleep((qoSFilterCacheTtlS + 1) * 1000);
        
        // Send at least as many requests as the maximum (50) MAX_REQUESTS used in this test,
        // as they are decreased one by one
        
        WORKER_SESSION.setWorkerProperty(WORKER1_ID, WORKER_KEY_SLEEP_TIME, "0");
        WORKER_SESSION.reloadConfiguration(WORKER1_ID);
        for (int i = 0; i < 50; i++) {
            CLIENT_CLI.execute("signdocument", "-servlet", "/signserver/worker/" + WORKER1_NAME, "-data", "foo");
        }
    ...
    public void singleRequest() throws Exception {
        ...
        // when
        int response = CLIENT_CLI.execute("signdocument", "-servlet",
                          "/signserver/worker/" + WORKER1_NAME,
                          "-data", "foo");
        LOG.debug("Error out: " + CLIENT_CLI.getErr().toString());
        LOG.debug("Standard out: " + CLIENT_CLI.getOut().toString());
        assertEquals("response code", 0, response);
        // then
    ...
    /**
     * Test that a single request will not be queued by the QoSFilter using
     * ClientWS.
     */
    @Test
    public void singleRequestClientWS() throws Exception {
        LOG.info("singleRequestClientWS");
        // given
        applyQoSFilterProperties(QoSFilterPropertiesBuilder.builder().withFilterEnabled("true"));
        // when
        int response = CLIENT_CLI.execute("signdocument", "-workername",
                          WORKER1_NAME,
                          "-data", "foo",
                          "-protocol", "CLIENTWS",
                          "-host", "localhost");
    ...
    /**
     * Test that a request to the PriorityClientWS for a worker that is not
     * configured for high priority is rejected.
     * Note: WORKER2_ID is not in the priorities (see setupClass).
     */
    @Test
    public void singleRequestPriorityClientWSNotAllowed() throws Exception {
        LOG.info("singleRequestPriorityClientWSNotAllowed");
        // given
        applyQoSFilterProperties(QoSFilterPropertiesBuilder.builder().withFilterEnabled("true"));
        // when
        int response = CLIENT_CLI.execute("signdocument", "-workername",
                          ...
                          "-protocol", "CLIENTWS",
                          "-host", "localhost",
                          "-servlet", "/signserver/ClientWSService/PriorityClientWS");
        LOG.debug("Error out: " + CLIENT_CLI.getErr().toString());
        LOG.debug("Standard out: " + CLIENT_CLI.getOut().toString());
        // then
        assertEquals("response code", -2, response);
    }

    /**
     * Test that a single request will not be queued by the QoSFilter using
     * PriorityClientWS.
     *  Note: WORKER1_ID is at priority level 5 (see setupClass).
     */
    @Test
    public void singleRequestPriorityClientWSAllowed() throws Exception {
        LOG.info("singleRequestPriorityClientWSAllowed");
        // given
        applyQoSFilterProperties(QoSFilterPropertiesBuilder.builder().withFilterEnabled("true"));
        // when
        int response = CLIENT_CLI.execute("signdocument", "-workername",
