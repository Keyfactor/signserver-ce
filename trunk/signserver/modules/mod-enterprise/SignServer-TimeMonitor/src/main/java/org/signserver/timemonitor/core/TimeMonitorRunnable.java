...
public class TimeMonitorRunnable implements Runnable, StateHolder {
    ...
    private String lastNtpErrorMessage;
    ...
    protected void runRound() {
        ...
        if (!runConfig.isOriginal() || !appConfig.isSignServerManagedConfig()) {
            if (!runConfig.isDisabled() && !gotRateLimiting) {
                ...
                timeState = timeTask(result, firstQuery || configChanged);
    ...
    protected TimeState timeTask(final NTPDateResult ntpDate,
        ...
        if (ntpDate == null) {
            ...
            timeState = TimeState.UNKNOWN;
            String ntpErrorMessage = "";
            if (ntpDate.getErrorMessage() != null) {
                // Remove all time/PID info from the error message
                ntpErrorMessage = ntpDate.getErrorMessage().replaceAll("\\d\\d? [A-Z][a-z][a-z] \\d\\d:\\d\\d:\\d\\d", "").replaceAll("ntpdate\\[\\d+\\]", "");
            }
            if (!TimeState.UNKNOWN.equals(oldTimeState) && (ntpDate.getErrorMessage() == null || !ntpDate.getErrorMessage().equals(lastCommandErrorMessage))
                    || !ntpErrorMessage.equals(lastNtpErrorMessage) || forceLogging) {
                logError("Command failed (stratum: " + ntpDate.getStratum() + ", exit: " + ntpDate.getExitCode() + "): " + ntpDate.getErrorMessage());
                lastNtpErrorMessage = ntpErrorMessage;
            }
        } else {
    ...
    protected void setNewRunConfig(TimeMonitorRuntimeConfig newRunConfig) {
        this.newRunConfig = newRunConfig;
    }
