...
public class NTPDateParserTest extends TestCase {
    ...
    public void testParse() {
        ...
        result = instance.parse(0, null, Arrays.asList("server 192.168.14.200, stratum 2, offset 0.001167, delay 0.12317",
                " 1 Nov 20:43:51 ntpdate[4529]: adjust time server 192.168.14.200 offset 0.001167 sec"));
        assertEquals("exit code", 0, result.getExitCode());
        ...
        result = instance.parse(0, null, Arrays.asList("server 192.168.15.200, stratum 0, offset 0.000000, delay 0.00000",
                ...
                " 1 Nov 20:43:51 ntpdate[4529]: adjust time server 192.168.16.200 offset 0.004711 sec"));
        assertEquals("exit code", 0, result.getExitCode());
        ...
        // Test response with serveral valid responses, should pick the one chosen by ntpdate
        result = instance.parse(0, null, Arrays.asList("server 192.168.15.200, stratum 3, offset 0.005781, delay 0.03389",
                "server 192.168.16.200, stratum 2, offset 0.004711, delay 0.04478",
                " 1 Nov 20:43:51 ntpdate[4529]: adjust time server 192.168.15.200 offset 0.005781 sec"));
        assertEquals("exit code", 0, result.getExitCode());
        ...
        // Test response with serveral valid responses, should pick the one chosen by ntpdate not the first valid response
        result = instance.parse(0, null, Arrays.asList("server 192.168.15.200, stratum 3, offset 0.005781, delay 0.03389",
                "server 192.168.16.200, stratum 2, offset 0.004711, delay 0.04478",
                "server 192.168.17.200, stratum 3, offset 0.005781, delay 0.03389",
                "server 192.168.18.200, stratum 5, offset 0.008981, delay 0.08789",
                " 1 Nov 20:43:51 ntpdate[4529]: adjust time server 192.168.18.200 offset 0.005781 sec"));
        assertEquals("exit code", 0, result.getExitCode());
        assertEquals("server", "192.168.18.200", result.getServer());
        assertEquals("stratum", 5, result.getStratum());
        assertEquals("offset", 0.008981, result.getOffset());
        assertEquals("delay", 0.08789, result.getDelay());
        assertFalse("rate limiting status", result.isRateLimited());

        // Test response with second server not responding, should pick the one chosen by ntpdate
        result = instance.parse(0, null, Arrays.asList("server 192.168.15.200, stratum 3, offset 0.005781, delay 0.03389",
