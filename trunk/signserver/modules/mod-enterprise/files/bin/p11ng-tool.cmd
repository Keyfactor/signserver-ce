...
if "%SIGNSERVER_HOME%" == "" (
    set SIGNSRV_HOME=..
    rem It must work to call both as bin\p11ng-tool.cmd or from within bin
    if not exist p11ng-tool.cmd set SIGNSRV_HOME=.
) else (
    set SIGNSRV_HOME=%SIGNSERVER_HOME%
)

rem Find the JAR
for /f "tokens=*" %%a in ('dir /b /s %SIGNSRV_HOME%\lib\SignServer-P11NG-Common*.jar') do set JAR=%%a
...
set CLASSPATH=%SIGNSRV_HOME%;%SIGNSRV_HOME%\conf;%JAR%;%SIGNSRV_HOME%\res\cesecore;%SIGNSRV_HOME%\lib\ext\jdbc\jdbc.jar;%OPTIONAL_CLASSPATH%
rem echo %CLASSPATH%
...
set CLASS_NAME=org.signserver.p11ng.common.cli.P11NgCli
